...
# Periodic job: compute simple KS-test per numeric feature vs baseline
...
def detect_drift(baseline_path, current_path):
    ...
    numeric = base.select_dtypes(include=[np.number]).columns.intersection(
        cur.select_dtypes(include=[np.number]).columns
    )
    report = {}
    for col in numeric:
